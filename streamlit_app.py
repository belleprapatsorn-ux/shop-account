...
# --- 3. ส่วนหน้าจอหลัก (UI) ---
st.title("💰 ระบบบัญชีร้าน Bell (จัดหมวดใหม่)")
...
with tab2:
    ...
    with st.form("expense_form", clear_on_submit=True):
        ...
        # --- LOGIC จัดกลุ่มใหม่ตรงนี้ ---
        expense_structure = {
            "1. 🥬 จ่ายตลาดสด (ของสดรายวัน)": ["ไก่สด", "ตีนไก่", "เส้นก๋วยเตี๋ยว", "ผักสด", "ของสดอื่นๆ"],
            "2. 📦 สั่งของเข้าร้าน (Online/ห้าง)": ["แมคโคร (Makro)", "Shopee (ซื้อของ)", "Lazada (ซื้อของ)", "Grab Mart", "LineMan Mart", "พริกน้ำตาลซอง"],
            "3. 🔥 ต้นทุนผลิต & แรงงาน": ["ค่าแก๊ส", "ค่าจ้างทำน้ำก๋วยเตี๋ยว", "ค่าจ้างลูกน้องหน้าร้าน"],
            "4. 🏠 ค่าที่ & ธรรมเนียม (Fixed Cost)": ["ค่าเช่าที่", "ค่าน้ำ/ค่าไฟ", "ค่าส่งของคืนลูกค้า", "ค่าใช้จ่ายอื่น ๆ"]
        }
...
with tab4:
    ...
    if not st.session_state.df.empty:
        ...
        st.download_button("📥 ดาวน์โหลด Excel (CSV)", csv, f"ShopAcc_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv")
